...
{
  if (i2c_status != I2C_OK) { log_e("I2C Error: %d", i2c_status); }
...
Status_t I2Cx_Master_Read(I2C_TypeDef *I2Cx,
                          uint8_t      slaveAddr,
                          void        *buf,
                          uint32_t     length)
{
  ...
  hi2c.i2cx = I2Cx;
  i2c_status =
      i2c_master_receive(&hi2c, slaveAddr, buf, length, I2C_TIMEOUT_DEFAULT_MS);
  return I2C_ConvertStatus(i2c_status);
}

Status_t I2Cx_Master_Write(I2C_TypeDef *I2Cx,
                           uint8_t      slaveAddr,
                           void        *buf,
                           uint32_t     length)
{
  ...
  i2c_status = i2c_master_transmit(
      &hi2c, slaveAddr, buf, length, I2C_TIMEOUT_DEFAULT_MS);
  return I2C_ConvertStatus(i2c_status);
}
