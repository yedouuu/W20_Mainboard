...
#define CLOCK_MAP_DEF(n)                                                       \
  {                                                                            \
    TMR##n, CRM_TMR##n##_PERIPH_CLOCK                                          \
  }

  const crm_tmr_clock_map_t clock_map[] = {
      CLOCK_MAP_DEF(1),
      CLOCK_MAP_DEF(2),
      CLOCK_MAP_DEF(3),
      CLOCK_MAP_DEF(4),
  ...
  for (index = 0; index < sizeof(clock_map) / sizeof(crm_tmr_clock_map_t);
       index++)
  {
    if (TIMx == clock_map[index].tmr)
    {
      crm_periph_clock_enable(clock_map[index].crm_periph_clock,
                              Enable ? TRUE : FALSE);
    }
  }
}
...
/**
 * @brief  频率因数分解，获取接近的值
 * 根据输入的频率和时钟，计算出两个因数，使得
 * freq = clock / (factor1 * factor2)
 * @param  freq: 中断频率(Hz)
 * @param  clock: 定时器时钟
 * @param  *period: 重装值地址
 ...
 * @retval 误差值(Hz)
 */
static bool Timer_FreqFactorization(uint32_t  freq,
                                    uint32_t  clock,
                                    uint16_t *factor1,
                                    uint16_t *factor2,
                                    int32_t  *error)
{
  ...
  if (freq == 0 || freq > clock) { return false; }
 ...
 * @retval 无
 */
static void Timer_TimeFactorization(uint32_t  time,
                                    uint32_t  clock,
                                    uint16_t *factor1,
                                    uint16_t *factor2)
{
 ...
 * @retval 无
 */
void Timer_SetInterrupt(TIM_TypeDef             *TIMx,
                        uint32_t                 Time,
                        Timer_CallbackFunction_t Function)
{
  ...
  /*将定时中断时间转换为重装值和时钟分频值*/
  Timer_TimeFactorization(Time, clock, &period, &prescaler);

  // printf("Timer_SetInterrupt: Time=%u, Clock=%u, Period=%u,
  // Prescaler=%u\r\n", Time, clock, period, prescaler);

  /*定时中断配置*/
  Timer_SetInterruptBase(TIMx,
                         period,
                         prescaler,
                         Function,
...
bool Timer_SetInterruptFreqUpdate(TIM_TypeDef *TIMx, uint32_t Freq)
{
  ...
  bool success =
      Timer_FreqFactorization(Freq, clock, &period, &prescaler, &error);
 ...
 * @retval 无
 */
void Timer_SetInterruptBase(TIM_TypeDef             *TIMx,
                            uint16_t                 Period,
                            uint16_t                 Prescaler,
                            Timer_CallbackFunction_t Function,
