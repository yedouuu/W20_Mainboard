...
typedef enum
{
  LCD_SHINY_ST043A23_IPS,          // 首荣 480x272 IPS 4.3寸屏, ST7796, mcu/8080 接口, 使用XMC连接
} LCD_Model_e;
...
const struct _LCD_Resource_t lcd_main_res = 
{
  ...
  .bus.xmc = 
  {
    ...
    .d_pins     = { // D0-D7
      SCREEN_XMC_D0_PIN, SCREEN_XMC_D1_PIN, SCREEN_XMC_D2_PIN, SCREEN_XMC_D3_PIN, \
      SCREEN_XMC_D4_PIN, SCREEN_XMC_D5_PIN, SCREEN_XMC_D6_PIN, SCREEN_XMC_D7_PIN,
    },
...
void __BSP_LCD_ST043A23_Param_Init(const struct _LCD_Resource_t* lcd_res)
{
  ...
  LCD_WR_COMMAND(lcd_res, 0xc1); // avdd_clp_en avdd_clp[1:0] avcl_clp_en avcl_clp[1:0]
  ...
  LCD_WR_DATA(lcd_res, 0x10);
  LCD_WR_COMMAND(lcd_res, 0xc6); // avdd_ratio_sel avcl_ratio_sel vgh_ratio_sel[1:0] vgl_ratio_sel[1:0]
  LCD_WR_DATA(lcd_res, 0x3e);
  LCD_WR_COMMAND(lcd_res, 0xc7); // mv_clk_sel[1:0] avdd_clk_sel[1:0] avcl_clk_sel[1:0]
...
inline void LCD_WR_COMMAND(const struct _LCD_Resource_t* lcd_res, uint8_t command)
{
  switch(lcd_res->bus_type)
  {
    case LCD_BUS_TYPE_GPIO: break;
    case LCD_BUS_TYPE_XMC:
      *(__IO uint8_t *) lcd_res->bus.xmc.cmd_addr = command;
      break;
    case LCD_BUS_TYPE_I2C: break;
    case LCD_BUS_TYPE_SPI: break;
    case LCD_BUS_TYPE_QSPI: break;
    case LCD_BUS_TYPE_UART: break;
  }
}
...
void BSP_LCD_DrawPoint(const void* lcd, \
