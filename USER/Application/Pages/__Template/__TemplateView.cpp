...
// LV_IMG_DECLARE(Bankosa__Template);

void __TemplateView::Create(lv_obj_t *parent)
{
  // // 2. 创建一个图片对象
  // lv_obj_t* img = lv_img_create(parent);

  // // 3. 设置图片源
  // lv_img_set_src(img, &Bankosa__Template);
...
void __TemplateView::SiderCreate(lv_obj_t *parent)
{
  // 创建侧边栏的容器
...
void __TemplateView::TopInfoCreate(lv_obj_t *parent)
{
  // 创建顶部信息区域的容器
...
void __TemplateView::ContentCreate(lv_obj_t *parent)
{
  // 创建内容区域的容器
  lv_obj_t *content = lv_obj_create(parent);
  // lv_obj_remove_style_all(content);
  lv_obj_set_size(content, lv_pct(100), lv_obj_get_height(parent) - 100);
  ...
  LV_LOG("【DEBUG】Content x: %d  y: %d\r\n",
         lv_obj_get_x(content),
         lv_obj_get_y(content));
...
void __TemplateView::BottomInfoCreate(lv_obj_t *parent)
{
  // 创建底部信息区域的容器
  ...
  lv_label_set_text(info_label, "Footer Information");
  lv_obj_center(info_label);
