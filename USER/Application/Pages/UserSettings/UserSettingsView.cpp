...
void UserSettingsView::Create(lv_obj_t *parent)
{
  // // 2. 创建一个图片对象
...
void UserSettingsView::SiderCreate(lv_obj_t *parent)
{
  // 创建侧边栏的容器
  ...
  // 设置容器样式（可选）
  lv_obj_set_style_bg_color(sider, lv_color_hex(0x000000), 0);
...
void UserSettingsView::TopInfoCreate(lv_obj_t *parent)
{
  // 创建顶部信息区域的容器
  ...
  // 设置容器样式（可选）
  lv_obj_set_style_bg_color(top_info, lv_color_hex(0x3e3e3e), 0);
  lv_obj_set_style_border_width(top_info, 0, 0);

  // 在顶部信息区域添加标签
  lv_obj_t *info_label = lv_label_create(top_info);
  lv_label_set_text(info_label, "UserSettings Header");
  lv_obj_center(info_label);
}

void UserSettingsView::ContentCreate(lv_obj_t *parent)
{
  // 创建内容区域的容器
  ...
  // 设置容器样式（可选）
  lv_obj_set_style_bg_color(content, lv_color_hex(0x0FFFFF), 0);
  lv_obj_set_style_border_width(content, 0, 0);

  // 在内容区域添加其他UI元素
  // 例如，添加一个标签
  lv_obj_t *content_label = lv_label_create(content);
  lv_label_set_text(content_label, "UserSettings Content Area");
  lv_obj_center(content_label);
}

void UserSettingsView::BottomInfoCreate(lv_obj_t *parent)
{
  // 创建底部信息区域的容器
  ...
  lv_obj_set_style_bg_color(settings_btn, lv_color_hex(0x00FF00), 0);
